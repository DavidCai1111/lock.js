...
;(function () {
  ...
  fs.stat = function (_path, callback) {
    ...
    if (!keysWithPrefix) return originalStat.call(fs, _path, callback)

    return process.nextTick(callback, null, new Stats(relativePath))
  }
  ...
  fs.statSync = function (_path) {
    ...
    if (!keysWithPrefix) return originalStatSync.call(fs, _path)

    return new Stats(relativePath)
  }
