...
;(function () {
  ...
  fs.exists = function (_path, callback) {
    let virtualPath = generateVirtualPath(_path)
  ...
  function ReadStream (_path, options) {
    ...
    this.end = options.end || Buffer.byteLength(filesMap.get(_path).content)
    this.bytesRead = 0
    this.pos = 0
    ...
    Buffer.isBuffer(filesMap.get(_path).content)
      ? this.buf = filesMap.get(_path).content.slice(this.start, this.end)
      : this.buf = new Buffer(filesMap.get(_path).content).slice(this.start, this.end)
  }
