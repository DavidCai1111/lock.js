...
;(function () {
  ...
  Object.getPrototypeOf(module).constructor._findPath = function (request, paths, isMain) {
    if (request.endsWith('/')) request = removeTrailingSlash(request)
  ...
  fs.readFileSync = function (_path, options) {
    ...
    let buffer = new Buffer(filesMap.get(virtualPath))
  ...
  function Stats (key) { this.key = key }
  Stats.prototype.isFile = function () { return filesMap.has(this.key) }
  Stats.prototype.isDirectory = function () { return !this.isFile() }
  ...
  fs.stat = function (_path, callback) {
    ...
    if (!find(filesMap, virtualPath)) return originalStat.call(fs, _path, callback)

    return process.nextTick(callback, null, new Stats(virtualPath))
  }
  ...
  fs.statSync = function (_path) {
    ...
    if (!find(filesMap, virtualPath)) return originalStatSync.call(fs, _path)

    return new Stats(virtualPath)
  }
...
function find (map, name) {
  return Array.from(map.keys()).find((key) => key === name)
}
