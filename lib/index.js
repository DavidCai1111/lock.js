...
lockjs
  ...
  .description('lock specified node.js project')
  .action(function (projectPath, entryPoint, dest) {
    co(lock(projectPath, dest, entryPoint)).catch(console.error)
  })

lockjs
  .command('unlock <packedProject> <dest> <publicKey>')
  .description('generate a executable from the locked node.js project')
  .action(function (packedProject, dest, publicKey) {
    co(function * () {
      ...
      unlock(packedProject, publicKey, dest, headersDir, sharedLibDir)
    }).catch(console.error)
  })
