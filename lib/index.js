...
lockjs
  ...
  .action(function (packedProject, dest, publicKey, version) {
    co(function * () {
      if (!version) {
        version = 'latest'
        yield installSharedLib(version)
      }

      execSync(`${path.resolve(__dirname, '../node_modules')}/.bin/node-gyp configure --silent`)
...
lockjs
  ...
  .action(function (version) {
    co(installSharedLib(version)).catch(console.error)
  })

lockjs.parse(process.argv)

function * installSharedLib (version) {
  let versionsMap = yield utils.getSharedLibVersionsMap()
