...
module.exports = function * (src, publicKey) {
  ...
  try {
    ...
    yield Object.keys(fileHeader).map(function * (filePath) {
      ...
      filesMap.set(path.resolve(process.cwd(), filePath), utils.decrypt(fileBuffer, key))
  ...
  Object.getPrototypeOf(module).constructor._findPath = function (request, paths, isMain) {
    ...
    for (let p of paths) {
      ...
      let havePrefixKeys = utils.getHavePrefixKeys(filesMap, tryPath)
...
function tryPackage (tryPath, paths, filesMap) {
  ...
  let havePrefixKeys = utils.getHavePrefixKeys(filesMap, main)
