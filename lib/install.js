...
module.exports = function * () {
  ...
  let result = yield urllib.request('https://raw.githubusercontent.com/DavidCai1993/node-shared-libs/master/package.json')

  if (!String(result.res.statusCode).startsWith('2')) throw new Error(result.res.data.toString())

  let versionMap = JSON.parse(result.res.data.toString()).versionsMap
  let version = versionMap.latest
  let cppVersion = versionMap[versionMap.latest]
  ...
  if (utils.exists(`${dest}/${version}/shared`) && utils.exists(`${dest}/${version}/node-v${version}`)) {
  ...
  if (process.platform === 'darwin') {
    yield download(`${baseUrl}/darwin/libnode.${cppVersion}.dylib.tar.gz`, `libnode.${cppVersion}.dylib.tar.gz`)
  } else {
    if (process.arch === 'x64') {
