...
function getSharedVerisonsMap () {
  let pkgUrl = 'https://raw.githubusercontent.com/DavidCai1993/node-shared-libs/master/package.json'

  return new Promise(function (resolve, reject) {
    cp.exec(`curl ${pkgUrl}`, function (error, stdout, stderr) {
      if (error) return reject(error)
      try {
        return resolve(JSON.parse(stdout).versionsMap)
      } catch (error) {
        return reject(error)
      }
    })
...
function check (command) {
  try {
    ...
    throw new Error(`"${command}" not found in the environment , please install it first.`)
  }
}

function getRandom () { return Math.random().toString(36).slice(2) }

module.exports = { isDirectory, isFile, exists, encrypt, decrypt, download,
getSharedName, getSharedDownloadUrl, check, getRandom, getSharedVerisonsMap }
